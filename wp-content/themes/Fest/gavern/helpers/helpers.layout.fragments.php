...
function gk_post_meta($attachment = false) {
 	...
 	<aside class="meta">
	 	...
	 	<?php if(!(is_home() || is_search())) : ?>
	 		<?php if(get_post_type() != 'page' && $param_category && !is_attachment()) : ?>
	 		<dt class="category">
...
function gk_author($author_page = false, $return_value = false) {
	...
	if($authorinfo_mode != 'none' && $is_excluded) :
		...
		<?php if($return_value == true) : ?>
			<?php return true; ?>
		<?php else : ?>
				<section class="author-info<?php if($author_page) : ?> author-page<?php endif; ?>">
					<aside class="author-avatar">
						...
						<h3>
						    <a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author">
						        <?php printf( '%s', get_the_author_meta('display_name', get_the_author_meta( 'ID' )) ); ?>  
						    </a>
						</h3>
