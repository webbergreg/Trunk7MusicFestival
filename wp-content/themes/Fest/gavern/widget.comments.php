		...
		if (count($comments)) {			
			if(count($comments) > 0) {
				...
				for($i = 0; $i < count($comments); $i++) {
					...
					echo '<span class="gk-avatar">' . get_avatar($comments[$i]->comment_author_email, $avatar_size) . '</span>';
					echo '<strong>' . $comments[$i]->comment_author . '</strong>';
					echo '<p><a href="'.get_comment_link($comments[$i]->comment_ID).'">' . $this->comment_text($comments[$i]->comment_content, $word_count) . '</a><small>' . $comments[$i]->comment_date_gmt . '</small></p>';		
					echo '</li>';
				}
